...
void solve()
{
	...
	pm[0] = 0;
	for (int i = 1; i <= n; i++) pm[i] = max(pm[i - 1], a[i]);
	sm[n + 1] = 0;
	for (int i = n; i >= 1; i--) sm[i] = max(sm[i + 1], a[i]);
	ll sb = 0;
	for (int i = 1; i <= n; i++) sb += (ll)b[i];
	ll ans = sb + (ll)pm[n];
	for (int i = 1; i <= n; i++)
	{
		int ma = max(b[i], max(pm[i - 1], sm[i + 1]));
		ans = max(ans, (ll)ma + sb - b[i] + a[i]);
	}
	cout << ans << endl;
	return;
}
