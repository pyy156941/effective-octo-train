...
int bucb[2000001], buca[2000001], cnt[2000001];
int a[1000001], b[1000001];
void Solve()
{
	int n, m;
	multiRead(n, m);
	for (int i = 1; i <= n + m; i++) buca[i] = bucb[i] = cnt[i] = 0;
	for (int i = 1; i <= n; i++) cin >> a[i], buca[a[i]]++;
	for (int i = 1; i <= m; i++) cin >> b[i], bucb[b[i]]++;
	for (int i = 1; i <= n + m; i++)
	{
		if (!buca[i]) continue;
		for (int j = 1; (ll)i * j <= n + m; j++) cnt[i * j] += buca[i];
	}
	int a = 0, b = 0, ab = 0;
	for (int i = 1; i <= n + m; i++)
	{
