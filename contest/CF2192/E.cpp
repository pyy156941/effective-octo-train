...
struct Edge
{
	int to, id;
	bool st;
	Edge (int _to, int _id, bool _st) : to(_to), id(_id), st(_st) {}
};
...
void DFS(int cur)
{
	for (int i = head[cur]; i < adj[cur].size(); i = head[cur])
	{
		...
		if (!adj[cur][i].st) ans[adj[cur][i].id] = true;
		vis[adj[cur][i].id] = true;
		DFS(adj[cur][i].to);
	}
}
...
void Solve()
{
	int n;
	cin >> n;
	for (int i = 1; i <= n; i++) ans[i] = vis[i] = false, adj[i].clear(), head[i] = buc[i] = 0;
	...
	for (int i = 1; i <= n; i++)
	{
		...
		buc[b[i]]++;
		if (a[i] == b[i]) continue;
		adj[b[i]].pb(Edge(a[i], i, true));
		adj[a[i]].pb(Edge(b[i], i, false));
	}
	for (int i = 1; i <= n; i++)
	{
		if (buc[i] % 2)
	...
	int acnt = 0;
	for (int i = 1; i <= n; i++) if (ans[i]) acnt++;
	cout << acnt << endl;
