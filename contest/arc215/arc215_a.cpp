...
int a[200001], dif[200001];
void Solve()
{
	int n, k, l;
	multiRead(n, k, l);
	ll toleftend, torightend, maxdiff;
	for (int i = 1; i <= n; i++) cin >> a[i];
	sort(a + 1, a + n + 1);
	toleftend = a[1], torightend = l - a[n];
	maxdiff = a[n] - a[1];
	ll ans = 0, cursum = 0;
	for (int i = 1; i < n; i++) dif[i] = a[i + 1] - a[i];
	sort(dif + 1, dif + n);
	reverse(dif + 1, dif + n);
	ans = (ll)max(toleftend, torightend) + (ll)(k - 1) * (ll)(l - maxdiff);
	for (int i = 1; i < n; i++)
	{
		cursum += (ll)dif[i] / 2;
		if (i == k) 
		...
		toleftend += (ll)dif[i] / 2, torightend += (ll)dif[i] / 2;
		maxdiff -= (ll)dif[i];
		ans = max(ans, cursum + (ll)max(toleftend, torightend) + (ll)(k - i - 1) * (ll)(l - maxdiff));
	}
	cout << ans << endl;
	return;
}
