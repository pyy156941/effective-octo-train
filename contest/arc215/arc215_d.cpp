...
{
	if (m >= n) return 0;
	return (ll)fac[n] * Inv((ll)fac[m], mod) % mod * Inv((ll)fac[n - m], mod) % mod;
}
...
void Solve()
{
	...
	multiRead(n, m);
	fac[0] = fac[1] = 1;
	for (int i = 2; i <= n / 2 + m + 1; i++) fac[i] = (ll)fac[i - 1] * i % mod;
	ll ans = 0;
	int ol = n / 2 + 1, el = n + 1 - ol;
	ans = binom(ol - 1 + m, ol - 1) * binom(el + m, el) % mod;
	ans = (ans + binom(ol + m, ol) * binom(el - 1 + m, el - 1) % mod) % mod;
	ans = (ans - binom(ol - 1 + m, ol - 1) * binom(el - 1 + m, el - 1) % mod + mod) % mod;
	cout << ans << endl;
	return;
}
