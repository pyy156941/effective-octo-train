...
void DFS(int cur)
{
	for (auto [it, weight] : adj[cur])
	{
		if (fa[cur] == it) continue;
		fa[it] = cur;
		dep[it] = dep[cur] + (ll)weight;
		DFS(it);
	}
}
...
void Solve()
{
	...
	DFS(1);
	int n1 = 0;
	for (int i = 1; i <= n; i++) if (dep[i] > dep[n1]) n1 = i;
	for (int i = 1; i <= n; i++) fa[i] = dep[i] = 0;
	DFS(n1);
	int n2 = 0;
	for (int i = 1; i <= n; i++) if (dep[i] > dep[n2]) n2 = i;
	cout << 2ll * su - dep[n2] << endl;
	return;
}
