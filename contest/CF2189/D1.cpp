...
void Solve()
{
	...
	string str;
	multiRead(n, c, str);
	ll ans_c = 2 % c, ans_mod = 2;
	for (int i = 0; i < n; i++)
	{
		if (str[i] == '0') w[i + 1] = false;
		else w[i + 1] = true; 
	}
	if (!w[1] || !w[n])
	{
	...
	for (int i = 2; i < n; i++)
	{
		...
		{
			ans_c = (ans_c * (ll)(i - 1)) % c;
			ans_mod = (ans_mod * (ll)(i - 1)) % mod;	
		}
	}
	// cerr << ans_c << ' ' << ans_mod << endl;
	if (!ans_c) cout << -1 << endl;
	else cout << ans_mod << endl;
	return;
}
