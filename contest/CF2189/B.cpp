...
{
	multiRead(n, x);
	for (int i = 1; i <= n; i++) multiRead(a[i], b[i], c[i]);
	for (int i = 1; i <= n; i++) val[i] = (ll)a[i] * b[i] - c[i];
	ll cur = 0;
	for (int i = 1; i <= n; i++) cur += (ll)a[i] * (b[i] - 1ll);
	if (cur >= x) 
	{
		cout << 0 << endl;
		return;
	...
	ll mv = 0;
	for (int i = 1; i <= n; i++) mv = max(mv, val[i]);
	if (!mv)
	{
		cout << -1 << endl;
		return;
	}
	ll ans = (x - cur + mv - 1ll) / mv;
	cout << ans << endl;
	return;
}
