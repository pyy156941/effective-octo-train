// Problem: E - Sum of Square of Sum
// Contest: AtCoder - AtCoder Beginner Contest 471
// URL: https://atcoder.jp/contests/abc471/tasks/abc471_e
...
void solve()
{
	...
	for (int i = 1; i <= n; i++)
	{
		ans = (ans + a[i] * a[i] % mod * binom(n - 1, k - 1) % mod) % mod;
		sum = (sum + a[i]) % mod;
	}
	for (int i = 1; i <= n; i++) ans = (ans + (sum - a[i] + mod) % mod * a[i] % mod * binom(n - 2, k - 2) % mod) % mod;
	cout << ans << endl;
	return;
}
