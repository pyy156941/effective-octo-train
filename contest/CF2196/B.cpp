// Problem: B. Another Problem about Beautiful Pairs
...
void Solve()
{
	...
	{
		for (int j = 1; j <= n; j++) pl[j] = 0;
		for (int j = n; j >= 1; j--) 
		{
			if (a[j] >= n) continue;
			if (a[j] == i) ans += (ll)pl[j]; 
			if (j - a[j] * i <= 0) continue;
			pl[j - a[j] * i]++; 
			// cerr << i << ' ' << j << ' ' << j - a[j] * i << endl;
		}
		// cerr << ans << endl;
	}
	for (int i = 1; i <= n; i++)
