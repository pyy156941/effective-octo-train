...
void solve()
{
	...
	for (int i = 1; i <= n; i++) cin >> a, buc[a]++;
	for (int i = 1; i <= n; i++) pre[i] = pre[i - 1] + buc[i];
	int ans = 1;
	for (int i = 2; i <= n; i++)
	{
		int erase = pre[i - 1];
		erase += pre[min(2 * i - 1, n)] - pre[i];
		if (2 * i < n) erase += pre[min(3 * i - 1, n)] - pre[2 * i];
		if (3 * i < n) erase += pre[min(4 * i - 1, n)] - pre[3 * i];
		if (erase <= k) ans = max(ans, i);
	}
	cout << ans << endl;
	return;
}
