...
void Solve()
{
	...
	for (int i = 0; i < (1 << c); i++) fail[i] = false;
	int mask = 0;
	for (int i = 1; i <= k; i++)
	{
		cnt[s[i]]++;
	...
	fail[((1 << c) - 1) ^ mask] = true;
	for (int i = k + 1; i <= n; i++)
	{
		cnt[s[i - k]]--;
		if (!cnt[s[i - k]]) mask ^= (1 << s[i - k]);
		cnt[s[i]]++;
		mask |= (1 << s[i]);
		fail[((1 << c) - 1) ^ mask] = true;
	}
	for (int i = (1 << c) - 1; i; i--)
	{
		...
		for (int j = 0; j < c; j++)
		{
			if (i & (1 << j)) fail[i ^ (1 << j)] = true;
		}
	}
	// for (int i = 0; i < (1 << c); i++) cerr << i << ' ' << fail[i] << ' ';
	// cerr << endl;
	int ans = c + 1;
	for (int i = 0; i < (1 << c); i++)
	{
		if (!(i & (1 << s[n]))) continue;
		if (!fail[i]) ans = min(ans, ppc(i)); 
	}
	cout << ans << endl;
	return;
}
