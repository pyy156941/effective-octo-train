...
void Solve()
{
	int n;
	cin >> n;
	for (int i = 1; i <= n; i++) multiRead(s[i].l, s[i].r);
	sort(s + 1, s + n + 1);
	ll sum_r = 0, sum_l = 0;
	int min_r = 2e9, max_l = 0;
	ll ans = 0;
	for (int i = 1; i <= n; i++) ans += (ll)(s[i].r - s[i].l);
	for (int i = 1; i <= n / 2; i++) sum_r += (ll)s[i].r, min_r = min(min_r, s[i].r);
	for (int i = n; i > n - n / 2; i--) sum_l += (ll)s[i].l, max_l = max(max_l, s[i].l);
	ans += sum_r - sum_l;
	if (n % 2)
	{
		int ans_p2 = 0;
		ans_p2 = max(ans_p2, s[n / 2 + 1].r - min_r);
		ans_p2 = max(ans_p2, max_l - s[n / 2 + 1].l);
		ans += ans_p2;
	}
	cout << ans << endl;
	return;
