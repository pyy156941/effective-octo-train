...
struct Method
{
	...
	Method(vector <int> p_) : p(p_), val(0)
	{
		for (int i = 0; i < n; i++) val += (ll)p_[i] * a[i];
	}
	
	bool operator < (const Method _) const
	{
		if (val == _.val)
		{
			for (int i = 0; i < n; i++)
			{
				if (p[i] != _.p[i]) return p[i] < _.p[i];
			}
			return false;
		}
		else return val < _.val;
	}
	
	bool operator == (const Method _) const
	{
		if (val != _.val) return false;
		for (int i = 0; i < n; i++) if (p[i] != _.p[i]) return false;
		return true;
	}
};

priority_queue <Method> Q;
void Solve()
{
	int k, x;
