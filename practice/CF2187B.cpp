// Problem: B. Shortest Statement Ever
...
void Solve()
{
	...
	for (int i = 30; i >= 0; i--)
	{
		...
		else if ((x & (1 << i)) == 0 && (y & (1 << i)) == 0) continue;
		pa += x & (1 << i), qa += y & (1 << i);
	}
	ans = abs(pa - x) + abs(qa - y);
	// cerr << pa << ' ' << qa << endl;
	if (!ans) 
	{
		cout << pa << ' ' << qa << endl;
	...
	for (int i = 30; i >= 0; i--)
	{
		...
		if ((x & (1 << i)) == 0 && (y & (1 << i)) == 0)
		{
			pb += (1 << i);
			cura = abs(pb - x);
			if (cura < ans) pa = pb, qa = y, ans = cura;
			pb -= (1 << i);
			qb += (1 << i);
			cura = abs(qb - y);
			if (cura < ans) pa = x, qa = qb, ans = cura;
			qb -= (1 << i);
		}
		else pb += x & (1 << i), qb += y & (1 << i);
	}
	cout << pa << ' ' << qa << endl;
	return;
}
